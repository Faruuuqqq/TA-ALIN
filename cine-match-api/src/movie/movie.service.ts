  ...
  private async loadAndVectorize() {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream
        ...
        .on('data', (row) => {
          ...
          try {
            ...
            const voteVal =
              row['vote_count'] ?? row['Vote_Count'] ?? row['voteCount'] ?? '0';
            ...
            if (genres.length === 0) {
              console.warn(
                `⚠️  Row ${rowCount}: No valid genres found, skipping.`,
              );
              errorCount++;
              return;
            }
        ...
        .on('end', () => {
          ...
          if (this.genreDimensions.length === 0) {
            return reject(new Error('❌ No genres found in dataset.'));
          }
      ...
      stream.on('error', (error: Error) => {
        reject(
          new Error(`❌ Stream error while reading dataset: ${error.message}`),
        );
      });
    });
  }
